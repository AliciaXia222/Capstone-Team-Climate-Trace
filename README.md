...
While increasing the number of clusters provides more granular information, the contribution of image-derived features remains relatively small, and the key socio-economic and climatic factors still dominate. Given the limited size of our dataset, with just over 400 rows, it may be more effective to work with fewer clusters to avoid overfitting and ensure that the model can generalize well with the available data. Therefore, using a smaller number of clusters, such as n=2, may be preferable for this context.
